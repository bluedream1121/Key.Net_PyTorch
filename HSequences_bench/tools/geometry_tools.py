...
import numpy as np ## For homography applying.
...
if __name__ =="__main__":
    import numpy as np
    import os
    path = 'datasets/synth/train_dataset_debug'
    keys = os.listdir('datasets/synth/train_dataset_debug/im_src_patch')

    for idx, key in enumerate(sorted(keys)):
        src_im = np.load(path+"/im_src_patch/"+key)
        trg_im = np.load(path+"/im_dst_patch/"+key)

        h_src_2_dst = np.load(path+'/homography_src_2_dst/'+key)
        h_dst_2_src = np.load(path+'/homography_dst_2_src/'+key)

        hom = prepare_homography(torch.from_numpy(h_dst_2_src)[0])
        shape_src = torch.randn(1,192,192).shape
        shape_dst = torch.randn(1,192,192).shape

        mask_src, mask_dst = create_common_region_masks(hom, shape_src, shape_dst)

        print(key, mask_src.numpy().shape, mask_dst.numpy().shape, torch.sum(mask_src), torch.sum(mask_dst))

    dst_pts = []
    a = torch.linspace(0,191, 192)#.astype(np.int)
    for i in a:
        for j in a:
            dst_pts.append([i,j, 1,1])
    dst_pts = torch.tensor(dst_pts)
    print(dst_pts.shape)

    dst_to_src_pts = apply_homography_to_points(dst_pts, hom)

    print(dst_to_src_pts)